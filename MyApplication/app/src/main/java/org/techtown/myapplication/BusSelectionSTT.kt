...
class BusSelectionSTT : AppCompatActivity(), TextToSpeech.OnInitListener {

    private lateinit var binding: ActivityBusSelectionSttBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityBusSelectionSttBinding.inflate(layoutInflater)
        setContentView(binding.root)

        // TextToSpeech 초기화
        tts = TextToSpeech(this, this)

        val busSelectionSTTIntent = getIntent()
        userId = busSelectionSTTIntent.getLongExtra("userId", -1L) // 기본값으로 -1L을 설정하여 오류 방지
        boardingStop = busSelectionSTTIntent.getStringExtra("boardingStop")
        Log.d("busSelectionSTTIntentCheck", "$boardingStop, $userId")

        // 음성인식 권한 설정
        requestPermission()

        // <말하기> 버튼 눌러서 음성인식 시작
        binding.speakButtonBusSelectionSTT.setOnClickListener {
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                ...
                // TTS 설정 완료 후 텍스트를 음성으로 출력
                speakOut("Please tell me the bus number")
            }
        } else {
    ...
    private val recognitionListener: RecognitionListener by lazy {
        object : RecognitionListener {
            override fun onReadyForSpeech(params: Bundle) {
                ...
                binding.tvStateBusSelectionSTT.text = "이제 말씀하세요!"
            }

            override fun onBeginningOfSpeech() {
                binding.tvStateBusSelectionSTT.text = "잘 듣고 있어요."
            }
            ...
            override fun onEndOfSpeech() {
                binding.tvStateBusSelectionSTT.text = "끝!"
            }
            ...
            override fun onError(error: Int) {
                val message = when (error) {
                    ...
                    SpeechRecognizer.ERROR_RECOGNIZER_BUSY -> {
                        runOnUiThread {
                            ...
                            speechRecognizer = SpeechRecognizer.createSpeechRecognizer(this@BusSelectionSTT)
                ...
                binding.tvStateBusSelectionSTT.text = "에러 발생: $message"
            }
            ...
            override fun onResults(results: Bundle) {
                ...
                runOnUiThread {
                    binding.textViewBusSelectionSTT.text = resultText
                    busRouteNm = resultText
                    Log.d("MainActivity", "Recognition result: $resultText")

                    Handler().postDelayed({
                        val busSelectionSTTIntent = Intent(this@BusSelectionSTT, DropOffBusStopSTT::class.java)
                        busSelectionSTTIntent.putExtra("boardingStop", boardingStop)
                        busSelectionSTTIntent.putExtra("userId", userId)
                        busSelectionSTTIntent.putExtra("busRouteNm", busRouteNm)
